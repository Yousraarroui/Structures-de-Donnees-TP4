...
void test_search(int num){
	...
	unsigned int max_op = 0, total_op = 0, min_op = 100000,nb_found = 0;

	SkipList sl = buildlist(num);

	char* search_file = gettestfilename("search",num);
	FILE* input = fopen(search_file, "r");
	if (input!=NULL) {
		...
		for (unsigned int i=0;i< nb_values; ++i) {
			...
			fscanf(input, "%d", &value);
			if (skiplist_search(sl,value,&nb_operations)){
				nb_found++;
				printf("%d -> true\n",value);
			}
			else printf("%d -> false\n",value);

			if (nb_operations<min_op) min_op = nb_operations;
			if (nb_operations>max_op) max_op = nb_operations;
			total_op += nb_operations;
		}
		printf("Statistics :\n	Size of the list : %d\n",skiplist_size(sl));
...
void test_search_iterator(int num){
	...
	if (input!=NULL) {
		...
		for (unsigned int i=0;i< nb_values; ++i) {
			...
			bool search = false;
			//recherche iterator
			for (SkipListIterator it = skiplist_iterator_create(sl,FORWARD_ITERATOR);
